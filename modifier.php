...
<form  action="" method="POST">
   <div class="loginbox">
      ...
      <h1>Entrer le nouveau compte</h1>
      <p>Nom</p>
...
<?php
     ...
     if(isset($_POST['valider'])){
      if (isset($_POST['nom']) && isset($_POST['pass'])) {
        $nom=$_POST['nom'];
        $pass=$_POST['pass'];
           $reponse = $bdd->exec('UPDATE accueil SET nom = "'.$nom.'" , mot_de_passe = "'.$pass.'" WHERE id = '.$id_bdd.' AND nom = "'.$_SESSION['nom_bdd'].'" ');
...
?>          
 ...
 <?php
  ...
  if ($cpt==1) {
    header('location:preModifier.php');
  }
